...
grep_job job_get_from_pool(p_grep grep) {
    grep_job job = NULL;
    while ((job = a_list_pop(grep->jobs_pool)) == NULL) {
        usleep(10);
    }
    return job;
}
...
int finish_file(p_grep grep, grep_file file) {
    int i;
    int ret = 0;
    qsort(file->result->array, file->result->size, sizeof(void*), compare_result);
    pthread_mutex_lock(&(grep->mutex));
    for(i=0;i<file->result->size;i++) {
...
int grep_read_next_file(p_grep grep) {
    char *next_file = a_list_pop(grep->files);
    //printf("Ler %s\n", next_file);
    grep_job job;
    grep_file file;
    int line = 0;
